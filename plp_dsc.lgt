...
:- uses(entity_writer_util,[write_object/4, write_clauses/2,listbodies_to_clauses/2,write_probfacts/2, write_plingo_problog_probfacts/2, write_flat/3, create_flat/3]).
...
write_plingo_problog(Identifier) :-
        atom_concat(Identifier, '.plp', Filename),
        open(Filename, write, Stream),
        findall(Fact-ProbString,(::probfact(Fact,Prob), atomics_to_string(['("',Prob,'")'],ProbString)),Probfacts),
        findall((Head :- Body),(::detrule(Head,Body)),Detrules),
        listbodies_to_clauses(Detrules,Clauses),
        write_clauses(Stream,Clauses),
        nl(Stream),
